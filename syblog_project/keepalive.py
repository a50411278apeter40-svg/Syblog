...
PING_INTERVAL = 180  # 3분

ENDPOINTS = ["/", "/blog/"]

def ping(url):
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'SyBlog-KeepAlive/1.0'})
        with urllib.request.urlopen(req, timeout=30) as resp:
            now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(f"[{now}] ✅ {url} → HTTP {resp.getcode()}", flush=True)
...
def main():
    ...
    while True:
        ...
        print(f"   다음 핑: {next_run.strftime('%H:%M:%S')}", flush=True)
